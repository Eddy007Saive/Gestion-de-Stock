...
export function EmployeeUpdate() {
  ...
  useEffect(() => {
    const fetchEmploye = async () => {
      try {
        const response1 = await getPostes();
        const response = await getEmployeById(id);
        setPoste(response1.data);
  ...
  const onSubmit = async (e) => {
    try {
      ...
      form.append("image-Update", image);
      for (let key in formData) {
        form.append(key, formData[key]);
      }
      const config = { headers: { "Content-Type": "multipart/form-data" } };
      const response = await updateEmploye(id, formData,config);
  ...
  value={formData.date_embauche ? new Date(formData.date_embauche).toISOString().split('T')[0] : ""}
  name="date_embauche"
  id="date_embauche"
  type="date"
  className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
...
/>
              ...
              <div className="w-full">
                ...
                <input
                  {...register("dateN")}
                  value={formData.dateN ? new Date(formData.dateN).toISOString().split('T')[0] : ""}
                  name="dateN"
                  id="dateN"
                  type="date"
                  className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
