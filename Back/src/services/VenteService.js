...
class VenteService {
    ...
    async createVentes(products, res) {
        ...
        try {
            for (const produit of products.panier) {
            ...
            const createdVentesDetails = await Promise.all(products.panier.map(async (produit) => {
            ...
            if (res && lastVenteId) {
                pdf = this.printFacture(lastVenteId,products.client, res);
            }
    ...
    async printFacture(id,client, res) {
        ...
        try {
            ...
            const factureData = {
                ...
                client: {
                    nom: vente.client?.nom ||client.nomClient,
                    adresse: vente.client?.adresse || client.adresse,
                    telephone: vente.client?.email || client.telephone,
